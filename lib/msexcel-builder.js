...
Sheet = (function() {
  ...
  Sheet.prototype.set = function(col, row, str) {
    if ((str != null) && str !== '') {
      //added check for string to be int..then do not add reference in shared strings
      if(isNaN(str)){
        return this.data[row][col].v = this.book.ss.str2id('' + str);
      }
      else
      {
        this.data[row][col].int = true;
        return this.data[row][col].v = str;
      }
    }
  };

  Sheet.prototype.setBoolean = function(col, row, str) {
      this.data[row][col].bool = true;
      return this.data[row][col].v = str;
  };

  Sheet.prototype.setDate = function(col, row, str) {
      ...
      this.data[row][col].date = true;
      return this.data[row][col].v = dateStr;
  };
  ...
  Sheet.prototype.autoFit = function(col){
    var maxLength = 0;
    for(var i = 1; i <= this.rows; i++){
      if(this.data[i][col].v){
        var text = this.data[i][col].v;
        if(this.data[i][col].int || this.data[i][col].bool || this.data[i][col].date ){
          //do nothing
        }
        else if(text < this.book.ss.arr.length){
          text = this.book.ss.arr[text - 1];
        }
    ...
    for (i = 1, _ref2 = this.rows; 1 <= _ref2 ? i <= _ref2 : i >= _ref2; 1 <= _ref2 ? i++ : i--) {
      ...
      for (j = 1, _ref3 = this.cols; 1 <= _ref3 ? j <= _ref3 : j >= _ref3; 1 <= _ref3 ? j++ : j--) {
        ...
        if ((ix.v !== 0) || (sid !== 1)) {
          ...
          if (ix.v !== 0) {
            //if int do not take reference from shared string
            if(ix.int){
              c.ele('v', ix.v);
            }
            else if(ix.bool){
              ...
              c.att('t', 'd');
              c.att('s', '' + this.book.st.mstyle.length); //as we have added date style at the end of mstyles
              c.ele('v', ix.v);
            }
            else{
              c.att('t', 's');
...
Style = (function() {
  ...
  Style.prototype.font2id = function(font) {
    ...
    font.name || (font.name = 'Calibri'); //宋体
  ...
  Style.prototype.toxml = function() {
    var bders, cs, e, ea, es, fills, fonts, o, ss, _i, _j, _k, _l, _len, _len2, _len3, _len4, _ref, _ref2, _ref3, _ref4, numFmts;
    ...
    numFmts.ele('numFmt',{
      numFmtId: 166,
      formatCode: "[$-F800]dddd\\,\\ mmmm\\ dd\\,\\ yyyy"
    });
    ...
    //add date number style cell style
    cs.ele('xf',{
      numFmtId: 166,
      fontId: 0,
      fillId: 0,
